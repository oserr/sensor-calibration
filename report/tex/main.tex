...
in an appropriate condition range. Based on such comprehensive understanding, 
we propose to develop a proof of concept which can be used to make a simple 
challenge-response authentication protocol between a car and the mobile device, 
using opportunistic calibration of the accelerometer.
\end{abstract}

\begin{IEEEkeywords}
Contextual security, sensor funsion, Madgwick, device orientation, deep learning,
...
context-aware authentication protocol that let's the application running on customer's 
...
The motivation of this work was from Convoy:Physical context verification for vehicle platoon 
...
Madgwick provided a novel orientation filter appicable to tri-axis gyroscopees and 
...
The basic logic of calculating the relative orientation was divide the quaternion representation with respect to each other, shown in eq.1. In quaternion representation you ca also perform the multiplication of inverse in place of division, shown in eq.2. Inverse of a quaternion is also called it's conjugate. So in eq.3, you can see the conjugate of $quaternion1$ is multiplied to $quaternion$, because we want to find the orientation of $quaternion1$ with respect to $quaternion$. In eq.4 we get the reoriented quaternion resentation of $quaternion1$.
\begin{align}
...
Fig3 gives the graphical representation of the yaw, pitch and roll orientation in terms of euler angles before applying sensor reorientation.
\begin{figure}[!t]\centering
  \includegraphics[width=5.5cm]{nonoriented}
  \caption{Euler Angles before reorientation.}\label{fig:fig3}
\end{figure}
Fig4 gives the graphical representation of the yaw, pitch and roll orientation in terms of euler angles after applying sensor reorientation.
\begin{figure}[!t]\centering
  \includegraphics[width=5.5cm]{orientd}
  \caption{Euler Angles after reorientation.}\label{fig:fig4}
\end{figure}
...
The reason for seecting three experiments was to introduce a clear pass case ($shared context$),
...
final project report and presentation; madgwick script modified in matlab; 
reorientation of quaternions, modified arduino sense to get sensor data from the 
android phone, literature review, conduct experiments.
